...
function header_motion(){
    ...
    $('.subnav').hover(function(){
        ...
        if ($('body').hasClass('gnb-hover')) {
          $(this).children('.subnav-content').stop().show();
        } else {
          $(this).children('.subnav-content').stop().slideDown({duration: 200, easing: "easeInQuad"});
        }
  ...
  //header 유틸
    ...
    $('.close-sch-btn').on('click',function(){
        ...
        $('.h-search-btn.focus-line-5').focus();
        setTimeout(function(){
            $('.h-sitemap-btn').show();
        }, 500);
  ...
  var userLang = document.documentElement.lang;
    ...
    $('.quickmenu ul').hover(function(){
        if (window.matchMedia('(min-width: 769px)').matches){
          if (userLang == 'en') {
            $('.quickmenu > ul').stop().animate({'width':'260px'}, 200 , 'easeOutBounce');
          }else {
...
function resetFile(data) {
    const filebox = data.closest('.multi-filebox');
    const uploadNameInput = filebox.querySelector('.upload-name');

    data.value = '';
    uploadNameInput.value = '첨부파일은 용량 10MB 이하만 가능합니다.';
    uploadNameInput.classList.remove('ch-c');
}

function insertFile(data) {
    const userLang = document.documentElement.lang;
    const allowExtensions = new Set([
        'pdf', 'jpg', 'gif', 'png', 'ppt', 'xls', 'doc', 'pptx', 'xlsx', 'docx', 'hwp', 'txt', 'zip'
    ]);
    ...
    const fileName = data?.value.replace('C:\\fakepath\\', '') ?? '';
    ...
    if (allowExtensions.has(extension)) {
        ...
        uploadNameInput.classList.add('ch-c');
    } else {
        if (userLang === 'ko') {
            alert('잘못된 파일입니다.');
        } else if (userLang === 'en') {
...
function kakaoMap(elemId, mTitle, mAddr, mTel){
    ...
    geocoder.addressSearch(mAddr, function(result, status) {
        ...
        if (status === kakao.maps.services.Status.OK) {
            ...
            var imageSrc = '/img/renew/content/ico-marker.svg',
...
function printDiv(divName) {
    ...
    myWindow.document.write('<link rel="stylesheet" href="/css/common_re.css" type="text/css" />');
