...
$(document).ready(function() {
    ...
    var popSDt = new Date('2023-12-07 00:00:00');
    var popEDt = new Date('2023-12-27 23:59:59');
    var popNDt = new Date();
    
    if(getCookie('importantPop1') != 'Y' && popNDt > popSDt && popNDt < popEDt){
        $('#importantPop1').show();
...
function main_media() {
  var userLang = document.documentElement.lang;
    ...
    if (window.matchMedia('(max-width: 768px)').matches) {
        $('.slick-wrap main-recruit-slider .slick-dots button').attr('tabindex', -1);
      ...
      if (userLang == 'ko') {
        ...
        // WA_LOB $('.main-recruit-slick-control button').attr('tabindex', 0);
...
function closeMainPop(key) {
        if($('#importantPopNo').prop('checked')) {
            setCookie('importantPop'+key, 'Y', 1);
        }
        $('#importantPop'+key).fadeOut();
